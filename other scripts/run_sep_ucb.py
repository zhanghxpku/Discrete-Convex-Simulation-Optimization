...
for N in range(70,160,10):
    ...
    f_out = open("./results/sep_ucb_" + str(params["N"]) + ".txt", "w")
    
    for t in range(400):
        print(t)
        model = models.separable_model.separable_model(params)
        ...
        gaps[0] = ( gaps[0] * t + model["f"]([output_ucb["x_opt"]]) - f_opt ) / (t+1)
        
        if model["f"]([output_ucb["x_opt"]]) - f_opt <= params["eps"]:
        ...
        f_out.write(" ".join( [str(output_ucb["total"]),\
                            str(model["f"]([output_ucb["x_opt"]]) - f_opt)] ))
        f_out.write("\n")
        f_out.flush()
