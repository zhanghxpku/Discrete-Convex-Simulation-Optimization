...
for t in range(1):
    print(t)
    model = models.queuereg_model_24hr(params)
...
#     # f_opt = model["f"](model["x_opt"])
    ...
    if method == 0:
        output_grad = solvers.gradient_proj_solver(model["F"],params,params["trunc"])
    elif method == 1:
        output_grad = solvers.vaidya_proj_solver(model["F"],params)
    elif method == 2:
        output_grad = solvers.random_walk_proj_solver(model["F"],params)
    elif method == 3:
        output_grad = solvers.dimension_reduction_proj_solver(model["F"], params)
    elif method == 4:
