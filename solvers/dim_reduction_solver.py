...
def DimensionReductionProjSolver(F,params):
    """
    ...
    # The initial uniform distribution in P
    # y_set = np.random.uniform(1,N,(d,math.ceil(50 * 20 * d * math.log(d+1) * max( 20, math.log(d) ))))
    y_set = E_inv @ np.random.uniform(1,N,(d,math.ceil(50 * 20 * d * math.log(d+1) * max( 20, math.log(d) ))))
    # The initial ellipsoid
    # C_inv = (N-1)**(-2) * 12 * np.eye(d) # A in paper
    # C = (N-1)**2 / 12 * np.eye(d) # A_inv in paper
    ...
    while d_cur > 1:
        ...
        # Iterate until we find a short basis vector
        # Use random walk method
        for K,z_new,A_new,b_new,y_new,s in RandomWalkProjApproximator(F,
                                                                C,y_set,A,
                                                                  b,params):
            # Check early stopping
            if K is None:
...
def RandomWalkProjApproximator(F,Y,y_in,A_in,b_in,params,centroid=False):
    """
    Output the new polytope, its approximate centroid and approximate covariance matrix.
