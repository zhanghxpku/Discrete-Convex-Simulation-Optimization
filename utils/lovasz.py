...
import numpy as np
from .subgaussian import required_samples, confidence_interval
...
def Lovasz(F, x, params, num_samples=1):
    ...
    # Local coordinate
    x_loc = x - x_int
    # Compute a consistent permutation
...
def Round(F, x, params):
    ...
    num_samples = required_samples(delta / 4, eps / 4, params)
    ...
    for i in range(num_samples):
        ...
        CI = confidence_interval(delta / 4, params, i + 1)
...
def SO(F, x, eps, delta, params):
    ...
    # Number of samples needed
    num_samples = required_samples(delta, eps / 2 / N / np.sqrt(d), params)
