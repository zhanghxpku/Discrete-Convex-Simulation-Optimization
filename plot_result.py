...
yfit1 = cs1(xfit)
plt.fill_between(xfit, yfit-3*yfit1, yfit+yfit1,color='b', alpha=0.1)
...
yfit1 = cs1(xfit)
plt.fill_between(xfit, yfit-3*yfit1, yfit+yfit1,color='r', alpha=0.1)

# Plot the trend
curve3, = plt.plot( N_set, 
                   np.ceil(np.log(0.5*N_set+11))*4.9e4+stat[0,0]-4.6e4*math.log(10),
                   alpha=0.2, color="b" )
curve4, = plt.plot( N_set, 4e4 * np.ones(N_set.shape), alpha=0.2, color="r" )

plt.legend([curve1,curve2,curve3,curve4],
           ["Adaptive","Uniform","y = O([log N])","y = Const"],
           loc='center right')
plt.xlabel("N")
plt.ylabel("Expacted simulation cost")
plt.title("Queueing model")
plt.savefig("./results/queue/queue.png")
